...
export class ExtendActiveEffectService {
    ...
    _onRenderActiveEffectConfig(controller, html, activeEffectContainer) {
        ...
        const renderNavigation = () => {
            ...
            if (!currentFeaturesNav) {
                ...
                featuresNavElement.childNodes.forEach(child => {
                    if (child.nodeName === '#text') {
                        child.textContent = ' ' + game.i18n.translations.DND5E.Features;
                    }
                });
            ...
            if (!currentSpellsNav) {
                ...
                spellsNavElement.childNodes.forEach(child => {
                    if (child.nodeName === '#text') {
                        child.textContent = ' ' + game.i18n.translations.DND5E.Spellbook;
                    }
                });
    ...
    _onCreateActiveEffect(parent, activeEffect, options, userId) {
    ...
    _onUpdateActiveEffect(parent, activeEffect, options, userId) {
        if (!Types.supportedAutoApplyParentTypes().includes(parent.data.type)) {
            return;
        }
        const actorData = JSON.parse(JSON.stringify(parent.data));
    ...
    _onDeleteActiveEffect(parent, activeEffect, options, userId) {
        if (!Types.supportedAutoApplyParentTypes().includes(parent.data.type)) {
            return;
        }
        const actorData = JSON.parse(JSON.stringify(parent.data));
    ...
    _onUpdateActor(actor, difference, options, userId) {
    ...
    _onPreDeleteOwnedItem(parent, ownedItem, options, userId) {
    ...
    _onPreUpdateOwnedItem(parent, ownedItem, difference, options, userId) {
        if (ownedItem.flags && ownedItem.flags[flagScope] && ownedItem.flags[flagScope].effectItemKey) {
            const effectId = ownedItem.flags[flagScope].effectItemKey.replace(/\.[0-9]+$/g, '');
            const activeEffect = WrappedActiveEffect.fromParameters(parent.data._id, effectId);
            if (activeEffect.isEnabled()) {
    ...
    _onUpdateOwnedItem(parent, ownedItem, difference, options, userId) {
        if (!Types.supportedAutoApplyParentTypes().includes(parent.data.type)) {
            return;
        }
        const actorData = JSON.parse(JSON.stringify(parent.data));
    ...
    _onDeleteOwnedItem(parent, ownedItem, options, userId) {
        if (!Types.supportedAutoApplyParentTypes().includes(parent.data.type)) {
            return;
        }
        const actorData = JSON.parse(JSON.stringify(parent.data));
    ...
    _calcApplyActorItems(actor) {
        const upsertItemsByKey = new Map();
        actor.effects.forEach((effect, effectId) => {
